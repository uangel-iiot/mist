  ...
  wrap([$class: 'AnsiColorBuildWrapper', 'colorMapName': 'XTerm']) {
    ...
    if ( env.BRANCH_NAME == 'master' ) {
      docker.withRegistry('https://index.docker.io/v1/', '2276974e-852b-45ab-bf14-9136e1b31217') {
        ...
        docker.image("hydrosphere/mist:${env.BRANCH_NAME}-${sparkVersion}").withRun("--rm --link ${mosquitto.id}:mosquitto --link ${hdfs.id}:hdfs") { c ->
      ...
      echo 'Testing Mist with Spark version: ' + sparkVersion
      docker.image("hydrosphere/mist:tests-${sparkVersion}").withRun("--rm --link ${mosquitto.id}:mosquitto --link ${hdfs.id}:hdfs -v ${env.WORKSPACE}:/usr/share/mist") { c ->
        sh "docker exec -i ${c.id} /docker-entrypoint.sh tests"
      }
    }
